...
/**
 * v4 default (unlabeled) batch associations — singular object names in path per HubSpot docs.
 * @param {string} token
 * @param {'contact'|'company'|'deal'} fromObjectType
 * @param {'contact'|'company'|'deal'} toObjectType
 * @param {string} fromId
 * @param {string} toId
 */
export async function associateV4DefaultBatch(token, fromObjectType, toObjectType, fromId, toId) {
  const path = `/crm/v4/associations/${fromObjectType}/${toObjectType}/batch/associate/default`;
